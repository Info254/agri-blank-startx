...
export default function WorkshopForm({ onCreated }) {
  ...
  const handleSubmit = async e => {
    ...
    setError('');
    try {
      const { data, error } = await createWorkshop(form);
      if (error) setError(error.message);
      else {
        setForm(initialState);
        notify({ type: 'workshop_new', title: 'New Workshop Posted', description: `${form.title}` });
        if (onCreated) onCreated(data);
      }
    } catch (err) {
      setError('Failed to submit workshop');
    }
    setLoading(false);
  };
  ...
  return (
    <form className="bg-white rounded shadow p-4 max-w-md mx-auto" onSubmit={handleSubmit}>
      <h2 className="text-lg font-bold mb-2">Post a Workshop</h2>
      <div className="mb-2">
